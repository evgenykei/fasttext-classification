 ...
 * The callback "tableLoaded" is called when loading is complete.
 */
EditableGrid.prototype.loadJSON = function(url, jsonHandler, callback, dataOnly)
{
	this.lastURL = url; 
	var self = this;
	...
	ajaxRequest.onreadystatechange = function () {
		...
		if (this.readyState == 4) {
			...
			if (jsonHandler) handledResponse = jsonHandler(this.responseText);
			if (!self.processJSON(handledResponse || this.responseText)) { console.error("Invalid JSON data obtained from url '" + url + "'"); return false; }
